...
esp_err_t camera_init(){
    ...
    /*if(CAM_PIN_PWDN != -1){
        ...
        io_conf.pin_bit_mask = CAM_PIN_PWDN;
    ...
    if (err != ESP_OK) {
        ESP_LOGE("CAM", "Failed to Initialize");
        return err;
    }
...
esp_err_t jpg_httpd_handler(httpd_req_t *req){
    ...
    ESP_LOGI("CAM", "JPG: %uKB %ums", (uint32_t)(fb_len/1024), (uint32_t)((fr_end - fr_start)/1000));
    return res;
}

esp_err_t jpg_ws_handler(httpd_req_t *req){
    camera_fb_t * fb = NULL;
    esp_err_t res = ESP_OK;
    size_t fb_len = 0;
...
esp_err_t jpg_ws_stream_handler(httpd_req_t *req){
    camera_fb_t * fb = NULL;
    esp_err_t res = ESP_OK;
    size_t _jpg_buf_len;
    uint8_t * _jpg_buf;
